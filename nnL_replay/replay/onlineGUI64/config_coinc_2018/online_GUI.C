...
TStyle Style2 = *gStyle;
TStyle Style3 = *gStyle;
#endif

// ==========================================================================================================================
//  Class: OnlineConfig (Utility class that reads in a text file (.cfg) and stores it's contents)
...
pair <UInt_t, UInt_t> OnlineConfig::GetPageDim(UInt_t page) 
{
	...
	UInt_t size1 = 2;
	if (IsLogy(page)||IsLogz(page)) size1 = 3;  // last word is "logy"
...
drawcommand OnlineConfig::GetDrawCommand(UInt_t page, UInt_t nCommand)
{
	...
	if(sConfFile[index].size()>=2) {
		...
		}  // joh 23-Feb-17: I'm guessing the position of this closing bracket
		// based on the indentation of the original, which the compiler
		// flags as ambiguous
	}
...
// ==========================================================================================================================
//  Class: OnlineGUI - Creates a GUI to display the commands used in OnlineConfig
	...
	fFileAlive(kFALSE)
{
	...
	if(printonly) {
		...
		fPrintOnly=kFALSE;
		CreateGUI(gClient->GetRoot(),800,1200);
	}
}
// -----------------------------------------------------------------------------------------------
void OnlineGUI::CreateGUI(const TGWindow *p, UInt_t w, UInt_t h) 
{
	...
	if(!goldenfilename.IsNull()) {
		...
		if( !goldenfilename.EndsWith(".root") )
			goldenfilename.Append(".root");

		TString basename = goldenfilename;
		TString tempfilename = goldenfilename;
		basename.Remove(basename.Length()-5);

		Long_t split = 0;

		while ( !gSystem->AccessPathName(tempfilename.Data()) && ++split ) {
			tempfilename = basename + "_" + split + ".root";
		}
		if( split == 0 ) {
			cerr << "ERROR: goldenrootfile: " << goldenfilename
				...
				<< endl;
			goto noGoldenFileAfterAll;
		}
		if( split > 1 ) {
			split--;
			goldenfilename = basename + "_" + split + ".root";
		}

		fGoldenFile.RootFile = new TFile(goldenfilename,"READ");
		if(!fGoldenFile.RootFile || !fGoldenFile.RootFile->IsOpen()) {
			cerr << "ERROR: Failed to open " << goldenfilename << endl;
			delete fGoldenFile.RootFile;
			goto noGoldenFileAfterAll;
		} else {
			// Open the Root Trees.  Give a warning if it's not there..
			...
			doGolden = kTRUE;
			cout << "Using Golden file: " << goldenfilename << endl;
		}
	} else {
noGoldenFileAfterAll:
		doGolden=kFALSE;
	...
	fTopframe = new TGHorizontalFrame(fMain,800,1000);
	fTopframe->SetBackgroundColor(mainguicolor);
	fMain->AddFrame(fTopframe, new TGLayoutHints(kLHintsExpandX 
				| kLHintsExpandY,10,10,10,1));

	// Create a vertical frame widget with radio buttons
	// This will hold the page buttons
	vframe = new TGVerticalFrame(fTopframe,40,1000);
	...
	if(!fConfig->IsMonitor()) {
		wile = 
			new TGPictureButton(vframe,
					gClient->GetPicture(guiDirectory+"/config_coinc_2018/src.png"));
	...
	fEcanvas = new TRootEmbeddedCanvas("Ecanvas", fTopframe, 760, 1000);
	...
	fBottomFrame = new TGHorizontalFrame(fMain,800,200);
	...
	hframe = new TGHorizontalFrame(fBottomFrame,800,40);
	...
	fPrintAll = new TGTextButton(hframe,"Print All");
	fPrintAll->SetBackgroundColor(mainguicolor);
	fPrintAll->Connect("Clicked()","OnlineGUI",this,"PrintAll()");
	hframe->AddFrame(fPrintAll, new TGLayoutHints(kLHintsCenterX,5,5,1,1));
...
void OnlineGUI::DoDraw() 
{
	...
	fCanvas->Clear();
	// comment by yang fCanvas->Divide(dim.first,dim.second);
	//yang
	TPaveText *numtitle = new TPaveText(0.4,0.955,0.6,1.0);
	numtitle->SetShadowColor(0);
	numtitle->AddText(Form("Run #%d",runNumber));
	numtitle->Draw();
	TPad *fpad = new TPad("pad","This is pad",0.0,0.05,1.0,0.95);
	fpad->Draw();
	fpad->Divide(dim.first,dim.second);
	//yang
...
// -----------------------------------------------------------------------------------------------
void OnlineGUI::MacroDraw(const drawcommand& command) {
...
// -----------------------------------------------------------------------------------------------
void OnlineGUI::CanvasDraw(const drawcommand& command, UInt_t padnum) {
...
// -----------------------------------------------------------------------------------------------
void OnlineGUI::GraphDraw(const drawcommand& command) {
	// Called by DoDraw(), this will draw the TCanvas in the current Canvas
...
// -----------------------------------------------------------------------------------------------
void OnlineGUI::HistDraw(const drawcommand& command) {
	...
	if(command.objtype.Contains("TH1")) {
		...
		if(!fRootFile.mytemp1d) {
			BadDraw(command.variable+" not found");
		} else if(fRootFile.mytemp1d->GetEntries()==0) {
			BadDraw("Empty Histogram");
		} else {
			if(showGolden) {
				Int_t fillstyle=3027;
				Double_t ngolden, nhist;
				fGoldenFile.RootFile->cd();
				if(fDir) fDir->cd();
				TH1D* h1 = (TH1D*)gDirectory->Get(command.objname);
				if( !h1 ) {
					cerr << "Can't find " << command.objname.Data() << " in golden run file" << endl;
					goto noGolden1DAfterAll;
				}
				if( h1->GetEntries()==0 ) {
					cerr << "Warning: Empty Histogram " << command.objname.Data()
						<< " in golden run file" << endl;
					goto noGolden1DAfterAll;
				}
				// Work with a temporary copy because we may want to scale it
				fGoldenFile.mytemp1d = new TH1D(*h1);
				...
				// Attempt to normalize the golden run's histogram to the current run's statistics
...
noGolden1DAfterAll:
	...
	if(command.objtype.Contains("TH2")) {
		...
		if(!fRootFile.mytemp2d) {
			BadDraw(command.variable+" not found");
		} else if(fRootFile.mytemp2d->GetEntries()==0) {
			BadDraw("Empty Histogram");
		} else {
			// Overlay of 2D histograms doesn't really make sense
			// if(showGolden) {
			// 	fGoldenFile.RootFile->cd();
			// 	if(fDir) fDir->cd();
			// 	fGoldenFile.mytemp2d = (TH2D*)gDirectory->Get(command.objname);
			// 	if(!fGoldenFile.mytemp2d) {
			// 	  cerr << "Can't find " << command.objname.Data() << " in golden run file" << endl;
			// 	  goto noGolden2DAfterAll;
			// 	}
	...
	if(command.objtype.Contains("TH3")) {
		...
		if(!fRootFile.mytemp3d) {
			BadDraw(command.variable+" not found");
		} else if(fRootFile.mytemp3d->GetEntries()==0) {
			BadDraw("Empty Histogram");
		} else {
			fRootFile.mytemp3d->Draw();
			// Overlay of 3D histograms doesn't really make sense either ...
			// if(showGolden) {
			// 	fGoldenFile.RootFile->cd();
			// 	if(fDir) fDir->cd();
			// 	fGoldenFile.mytemp3d = (TH3D*)gDirectory->Get(command.objname);
			// 	if(!fGoldenFile.mytemp3d) {
			// 	  cerr << "Can't find " << command.objname.Data() << " in golden run file" << endl;
			// 	  goto noGolden3DAfterAll;
			// 	}
...
void OnlineGUI::PrintToFile()
{
	...
	if(fi.fFilename!=NULL) {
		...
		if(!filename.EndsWith(fi.fFileTypes[fi.fFileTypeIdx+1]+1)) filename += fi.fFileTypes[fi.fFileTypeIdx+1]+1;
		fCanvas->Print(filename.Data());
	}
...
void OnlineGUI::PrintPages() {
	...
	for(UInt_t i=0; i<fConfig->GetPageCount(); i++) {
		...
		TString pagename = fConfig->GetPageTitle(current_page);
		lt->SetTextSize(0.02);
		lt->DrawLatex(0.06,0.95,pagename);
		if(useJPG) {
	...
	if(fi.fFilename!=NULL) {
		...
		if (filename!=TString::Format("%s/summaryfiles/temp_%d.pdf",sum_dir.Data(),runNumber)) {
			if ( gSystem->Exec(Form("mv %s/summaryfiles/temp_%d.pdf %s",sum_dir.Data(),runNumber,filename.Data()))==0 ) {
				...
				cerr << "  File " << filename << " is NOT generated.\n";
			}
		} else {
			cout << "  File " << filename << " is generated.\n";
		}
...
void UseMyStyle()
{
	Style1.SetOptStat(1110);
	Style2.SetOptStat(1110);
	Style3.SetOptStat(1110);
	Style1.SetStatFontSize(0.1);
	Style2.SetStatFontSize(0.1);
	Style3.SetStatFontSize(0.1);
	Style1.SetPalette(1);
	Style2.SetPalette(1);
	Style3.SetPalette(1);
	...
	Style3.SetTitleW(0.48);
	Style3.SetLabelSize(0.08,"X");
	Style3.SetLabelSize(0.08,"Y");
}
